...
export default function OuraRingCard() {
  ...
  useEffect(() => {
    ...
    if (params.get(PROVIDER) === 'connected') {
      toast.success('OuraRing connected! Syncing data...');
      queryClient.invalidateQueries({ queryKey: ['wearableStatus'] });
      window.history.replaceState({}, '', window.location.pathname);
    } else if (params.get(PROVIDER) === 'error') {
      toast.error('OuraRing connection failed. Ensure credentials are set in Railway.');
      window.history.replaceState({}, '', window.location.pathname);
    }
  }, []);
  ...
  const connectMutation = useMutation({
    mutationFn: async () => {
      ...
      else throw new Error(data.setup || data.error || 'OuraRing not configured');
    },
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess:  () => {
      toast.success('OuraRing disconnected');
      queryClient.invalidateQueries({ queryKey: ['wearableStatus'] });
    },
  });
  ...
  return (
    <ConnectedIntegrationCard
      logo="💍"
      name="Oura Ring"
      description="Sleep and readiness scores to optimize your nutrition"
      isConnected={status?.connected}
      lastSync={status?.lastSync}
      onConnect={() => connectMutation.mutate()}
      onDisconnect={() => disconnectMutation.mutate()}
      isLoading={connectMutation.isPending || disconnectMutation.isPending}
      connectLabel="Connect Oura Ring"
      connectedBenefits={['Readiness score adjusts meal plan', 'HRV data linked to anti-inflammatory recs', 'Sleep stages inform recovery nutrition']}
    />
  );
}
